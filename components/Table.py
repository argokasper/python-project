...
class Table(Frame):
    ...
    def __init__(self, container, data = [], headers = {}, *args, **kwargs):
        ...
        self.tree = Treeview(margin, columns=list(headers.keys()), height=400, selectmode='extended',
        ...
        for index, (key, header) in enumerate(headers.items()):
            self.tree.heading(key, text=header, anchor=W)
            self.tree.column('#{0}'.format(index), stretch=NO, minwidth=0, width=120)
        self.tree.pack()
        for row in data:
